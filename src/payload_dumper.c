...
#ifdef ENABLE_HTTP_SUPPORT
char *format_size(uint64_t bytes); // Forward declaration (implemented in
                                   // http_reader.c when HTTP is enabled)
#else
// Provide our own format_size implementation when HTTP is disabled
...
reader_t *open_payload_source(const char *source_path, const char *user_agent,
  ...
  if (strncmp(source_path, "http://", 7) == 0 ||
    ...
    // Skip metadata check and directly use ZIP parsing
      ...
      if (get_data_offset(reader, &payload_entry) == 0) {
        if (verify_payload_magic(reader, payload_entry.data_offset) == 0) {
          ...
          *payload_size =
          ...
          printf("- Found payload: offset=%lu, size=%s\n", *payload_offset,
    ...
    if (find_payload_entry(reader, &payload_entry) == 0) {
      if (get_data_offset(reader, &payload_entry) == 0) {
        if (verify_payload_magic(reader, payload_entry.data_offset) == 0) {
          ...
          *payload_size =
              payload_entry.uncompressed_size; // or compressed_size?
          printf("- Found payload in ZIP: offset=%lu, size=%s\n",
...
int extract_payload(const char *payload_path, const char *user_agent,
  ...
  if (file_format_version != 2) {
    printf("- Unsupported file format version: %lu\n", file_format_version);
    reader_cleanup(payload_reader);
    free(payload_reader);
    return -1;
  }
  ...
  for (int i = 0; i < active_threads; i++) {
    ...
    pthread_create(&threads[i], NULL, process_partition_thread,
                   &thread_data[i]);
  }
...
int main(int argc, char *argv[]) {
  ...
  if (num_threads <= 0 || num_threads > MAX_THREADS) {
    num_threads = 4; // fallback to 4 if detection fails
  }
